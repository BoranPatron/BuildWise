...
async def get_current_user(
    ...
    try:
        print(f"[DEBUG] [API-DEPS] get_current_user called - bypassing auth")
        ...
        if not user:
            print(f"[ERROR] [API-DEPS] User ID 2 not found in database")
        ...
        print(f"[ERROR] [API-DEPS] Error in get_current_user: {e}")
        import traceback
        traceback.print_exc()
