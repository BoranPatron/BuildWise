...
async def create_audit_log(db: AsyncSession, audit_data: AuditLogCreate) -> AuditLog:
    ...
    print(f"📝 Erstelle Audit-Log: {audit_data.action}")
    print(f"📊 Audit-Daten: {audit_data.dict()}")
    ...
    try:
        await db.commit()
        print(f"✅ Audit-Log erfolgreich in Datenbank gespeichert")
        await db.refresh(audit_log)
        print(f"📊 Audit-Log-ID: {audit_log.id}")
        return audit_log
    except Exception as e:
        print(f"❌ Fehler beim Speichern des Audit-Logs: {e}")
        await db.rollback()
        raise
...
async def get_audit_logs_for_user(db: AsyncSession, user_id: int, limit: int = 100) -> List[AuditLog]:
    ...
    print(f"📝 Lade Audit-Logs für User {user_id}")
    ...
    print(f"✅ {len(audit_logs)} Audit-Logs für User {user_id} gefunden")
    return audit_logs
...
async def get_audit_logs_for_project(db: AsyncSession, project_id: int, limit: int = 100) -> List[AuditLog]:
    ...
    print(f"📝 Lade Audit-Logs für Projekt {project_id}")
...
async def get_all_audit_logs(db: AsyncSession, limit: int = 1000) -> List[AuditLog]:
    ...
    print(f"✅ {len(audit_logs)} Audit-Logs insgesamt gefunden")
    return audit_logs 