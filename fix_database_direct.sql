-- Direktes SQL-Skript zur Behebung der Geo-Karten-Probleme
-- Führen Sie dieses Skript in Ihrer PostgreSQL-Datenbank aus

-- 1. Zeige aktuelle Projekte an
SELECT 
    id,
    name,
    address,
...
-- 2. Füge Adressen zu bestehenden Projekten hinzu
UPDATE projects 
...
-- 3. Stelle sicher, dass Projekte öffentlich sind
UPDATE projects 
SET is_public = true, allow_quotes = true
WHERE id IN (1, 2, 3, 4, 5);

-- 4. Zeige die aktualisierten Projekte an
SELECT 
    id,
    name,
    address,
...
-- 5. Zeige alle Milestones/Gewerke an
SELECT 
    m.id,
    m.title,
    m.category,
